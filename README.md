...
C# 12 / .NET 8 · Avalonia 11 · 테스트 474개 · 빌드 경고 0

![디지털 I/O 패널](docs/screenshots/01-digital-io.png)
...
> **주소 입력** — 한글 IME 가 켜져 있으면 전각 문자(`％ＭＷ０`)가 들어가 파싱이 실패한다.
> 화면에서 거의 같아 보여 원인을 알기 어려우므로 입력을 자동 정규화한다(전각→ASCII, 대문자화,
> 선행 `%` 보충). 그래도 실패하면 받은 문자의 **코드포인트를 표시**해 원인을 알려준다.
...
- [ ] **"디지털 I/O"** 탭에 주소를 추가해 토글해 본다 (`%MW0` 을 넣으면 16비트가 펼쳐진다)
...
- [ ] **"디지털 I/O"** 탭에 마스터가 읽거나 쓰는 주소를 추가한다 (예: `%MW0`, `%QW10`)
...
- [ ] 비트 토글 → LabVIEW 화면에 반영되는지 확인 (**읽기** 경로)
- [ ] LabVIEW에서 쓰기 → 같은 목록의 LED가 켜지는지 확인 (**쓰기** 경로)
...
### 디지털 I/O — 하나의 목록, 모든 점이 양방향

입력·출력을 나누지 않는다. **비트를 눌러 쓰면 마스터가 읽고, 마스터가 쓰면 표시가 따라온다.**
시뮬레이터에서는 사람이 PLC 프로그램 역할까지 하므로 `%I`·`%Q`·`%M` 어느 영역이든 양쪽 방향이 다 필요하다.
...
- 각 비트를 눌러 켜고 끈다 — `전체 ON` / `전체 OFF` 로 한 번에도 가능
- 마스터가 쓴 값은 LED 로 즉시 반영된다
...
![A/D 입력 패널](docs/screenshots/02-analog-input.png)
...
![주소 워치](docs/screenshots/03-watch.png)
...
![트래픽 로그](docs/screenshots/04-traffic-log.png)
...
```json
{
  ...
  "digitalPoints": [
    { "address": "%MW320", "label": "운전 지령 워드" },
    { "address": "%QW10", "label": "운전 상태 워드" }
  ],
  "analogPoints": [
...
임의 주소를 비트 배열로 펼쳐 **양방향으로** 읽고 쓴다. 비트·바이트·워드·더블워드·롱워드 모두
쓸 수 있고, 폭만큼 비트가 펼쳐진다. 입력/출력 구분이 없다.
...
| 사용자 지정 디지털 점 (임의 폭 · 비트 배열 · 양방향 단일 목록) | ✅ |
