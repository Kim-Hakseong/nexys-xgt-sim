...
- [ ] 한 값이 여러 주소에 함께 있어야 하면 **"묶음"** 탭에서 `%MW0 %MW1` 처럼 묶는다 (비트는 `%MW0.10 %MW1.10`)
- [ ] 마스터가 어느 주소를 쓰는지 모르겠으면 **"범위 보기"** 탭에서 `%MW0` · `100` 으로 펼쳐 두고 색이 들어오는 칸을 찾는다
- [ ] **"A/D 입력"** 탭에 센서 주소와 스케일을 넣고 공학단위 값을 입력해 본다
- [ ] raw 값이 이상하면(예: `999999961`) 그 채널의 형식을 `실수 Float (4B)` 로 바꿔 본다 — 마스터가 실수를 넣는 경우다
...
- [ ] **거절된 행을 클릭하면 아래에 그 프레임 전문이 펼쳐진다** — 거절 사유에 무엇이 왜 안 맞았는지 숫자로 적혀 있다. 원인이 안 잡히면 이 전문을 그대로 보내면 된다
- [ ] 한 주소만 추적하려면 트래픽 탭 **주소 필터**에 `%MW320` 을 넣는다 (여러 개 넣으면 그중 하나라도 걸리는 프레임을 본다)
- [ ] **방향** 콤보로 `RX + TX 함께` · `RX 만 (마스터 → 시뮬)` · `TX 만 (시뮬 → 마스터)` 중 하나를 고른다 — 마스터가 보낸 것만 보고 싶을 때 RX 만
...
채널마다 **raw 표시 형식**과 **바이트 순서**를 고를 수 있다(주소 워치와 같은 선택).
마스터가 워드에 정수를 넣는 게 보통이지만 IEEE754 실수를 넣는 경우도 있는데, 그때 정수로 읽으면
`999999961` 같은 값이 나와 스케일 환산이 무의미해진다. `실수 Float (4B)` · `실수 Double (8B)` 로
바꾸면 raw 자체가 실수가 되고 소수부도 보존된다. 형식만 바꾸는 것은 **메모리를 건드리지 않는다** —
같은 바이트를 다르게 읽을 뿐이다.
...
### 묶음 — 묶인 주소는 항상 같은 값을 갖는다

실장비에서는 PLC 프로그램이 "MW0 을 MW1 에 복사" 같은 로직을 돌리는데, 시뮬레이터에는 그 프로그램이 없다.
묶음이 그 자리를 메운다 — **한쪽에 값이 들어가면 나머지가 따라온다.** 방향이 없어 어느 쪽에 써도 같다.

| 하고 싶은 것 | 이렇게 넣는다 |
|---|---|
| 워드끼리 묶기 | `%MW0 %MW1` |
| 서로 다른 워드의 같은 비트 | `%MW0.10 %MW1.10` |
| **한 워드 안의 두 비트** | `%MW0.10 %MW0.12` |
| 더블워드(아날로그) 묶기 | `%MD100 %MD200` |
| 셋 이상 | `%MW0 %MW5 %MW9` |

`%MW1.10` 은 "MW1 의 10번째 비트" 다 — 절대 비트 번지(`%MX26`)를 직접 계산하지 않아도 된다.

- **디지털·아날로그 모두** 된다. 크기가 같은 주소끼리만 묶인다(다르면 무엇을 무엇에 복사할지 정할 수 없다)
- 마스터가 쓰든 화면에서 쓰든 전파된다 — 모든 쓰기가 같은 메모리를 지나기 때문이다
- 한 번의 쓰기가 같은 묶음의 멤버를 여러 개 덮으면 **가장 낮은 번지**가 원본이 된다
- 목록은 `.nxp` 에 함께 저장된다

![묶음](docs/screenshots/04-links.png)

### 범위 보기 — 주소를 하나씩 추가하지 않고 한꺼번에 본다

마스터가 **어느 주소를 건드리는지 모를 때** 쓰는 화면이다. 시작 주소와 개수만 정하면
그 구간을 통째로 펼쳐 놓는다. 개수는 `10` · `100` · `300` · `500` · `1000` 버튼으로 바로 고르거나
직접 입력한다(최대 4096).

- **방금 바뀐 칸에 색이 들어온다** — 랩뷰의 쓰기가 실제로 어디에 떨어지는지 이걸로 확인한다
- 표시 형식·바이트 순서를 바꾸면 펼친 칸 전체에 즉시 반영된다
- 칸을 고르면 그 자리에서 **값을 쓰거나** 계속 지켜볼 주소를 **워치로 옮길 수 있다**
- 수천 칸도 가상화 목록이라 버벅이지 않는다
- 칸 너비는 표시 형식이 낼 수 있는 **가장 긴 표기**에 맞춘다 — 2진 워드(`0000 0100 1101 0010`)도 잘리지 않는다

![범위 보기](docs/screenshots/05-range.png)

### 트래픽 로그 — raw hex + 해석 요약 + 주소·방향 필터

RX/TX 쌍, 거절 사유, 타임스탬프, **그 프레임이 건드린 주소**가 함께 남는다.

- **방향** — `RX + TX 함께` / `RX 만 (마스터 → 시뮬)` / `TX 만 (시뮬 → 마스터)` 3택
- **주소 필터** — 추적할 주소를 넣으면 그 주소를 건드린 프레임만 남는다. 여러 개는 OR 로 묶인다
- **오류만** 과 함께 걸 수 있다. 저장(**[⬇]**)도 지금 화면과 같은 필터를 적용해 내려간다
- **정렬 버튼**으로 최근을 위/아래 어느 쪽에 쌓을지 고른다. 기본은 **최근이 위**

주소 열은 요약 문자열을 문자열 검색하지 않는다 — 코덱이 파싱한 주소를 그대로 실어 보낸다.

#### 프레임 전문 해부

행을 클릭하면 아래에 그 프레임이 **구간별로 쪼개져** 펼쳐진다.

- 바이트 격자가 **헤더 / 변수명 / 값 / 미해석**을 배경색으로 구분한다 — 헤더가 어디까지인지 한눈에 보인다
- **주소 칩**(`%MD312` 등)을 누르면 그 주소가 쓰인 바이트가 블록으로 잡힌다. 쓰기 프레임이면 이름이 아니라 **값 바이트**를 잡는다
- **바이트를 눌러도** 그 바이트가 속한 구간이 잡히고, 구간 목록에서 골라도 같다
- 구간 목록은 위치(`14 ~ 15 (2바이트)`) · 이름 · 해석값을 함께 보여 준다.
  길이 필드가 실제 프레임과 다르거나 BCC 가 안 맞으면 그 자리에서 지적한다
- 깨진 프레임에서도 죽지 않는다 — 해석하지 못한 바이트는 버리지 않고 미해석 구간으로 남긴다
...
`format` 이 raw 를 어떤 형식으로 읽고 쓸지(`Signed` 기본 · `Decimal` · `Hex` · `Binary` ·
`Float` · `Double`), `order` 가 바이트 순서를 정한다. 비트 주소는 쓸 수 없다.
...
| 트래픽 로그 (RX/TX hex + 해석 + 주소·방향·오류 필터 + 프레임 해부 + 파일 저장) | ✅ |
| 주소 묶음 (워드·비트·더블워드 · 양방향 전파) | ✅ |
| 프로젝트 파일 (.nxp JSON) | ✅ |
